...
postData(
  `http://kgamifytest-env.eba-sb353hxm.ap-south-1.elasticbeanstalk.com/app/championships/all`
).then((data) => {
  if (JSON.stringify(data) == "{}") {
  ...
  async function dropdown() {
    let result = ``;
    let test = 0;
    while (test < data.championships.length) {
      result += `
       ...
       <br /><br />
        ...
        <div style="display: flex; flex-direction: row;">
            <div style=" display: flex; min-width: 112ch; font-size: 16px;" class="select">
                  <span style="font-size: 20px;font-weight:bold;"> ${data.championships[test].championship}  </span>
            </div>
        ...
        <div style="display: flex; flex-direction: row;">
          <div style=" display: flex; min-width: 508ch; font-size: 16px;"  class="select">
            <div  class="card-title">
              <p>Total Participants: ${data.championships[test].number_of_participants}</p>
              <p>Qualification Required: ${data.championships[test].qualification}</p>
              <p>Category: ${data.championships[test].category}</p>
              <p>Start Date: ${data.championships[test].start_time}</p>
            </div>
          </div>
        </div>

        <button onclick="edit('${data.championships[test].championship}')" style="background-color: #004ffa;padding:10px 20px 10px 20px;color:white;font-size: 15px;border:none;"  id="dodropdown"> <i class="fas fa-edit"></i> Edit</button>
        <button onclick="deactivate('${data.championships[test].championship}','${data.championships[test].category}')" style="background-color: #c90101;padding:10px 20px 10px 20px;color:white;font-size: 15px;border:none;"   id="dodropdown"><i class="fas fa-times"></i> Delete</button>
    ...
    async function dropdown() {
      ...
      while (test < data.Items.length) {
       ...
       <br /><br />
        ...
        <button onclick="edit('${data.Items[test].championship}')" style="color: #004ffa; margin: 2px; font-size: 15px; height: 30px;border:1px solid #ff9d5c;"  id="dodropdown"> <i class="fas fa-edit"></i> Edit</button>
        <button onclick="deactivate('${data.Items[test].championship}','${data.Items[test].category}')" style="color: #c90101; margin: 2px; font-size: 15px; height: 30px;border:1px solid #ff9d5c;"   id="dodropdown"><i class="fas fa-times"></i> Delete</button>
...
function deactivate(championship = "", category = "") {
  async function postData(url = "") {
    // Default options are marked with *
    const response = await fetch(url, {
  ...
  postData(
    `http://kgamifytest-env.eba-sb353hxm.ap-south-1.elasticbeanstalk.com/app/championships?championship=${championship}`
  ).then((data) => {
    if (JSON.stringify(data) == "{}") {
